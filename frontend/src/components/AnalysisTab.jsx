// AnalysisTab.jsx (Fixed for YARA matches format)
...
export default function AnalysisTab({ analysis }) {
  ...
  const processedIocs = analysis.iocs
  ...
  // Process YARA matches to handle both string and object formats
  const processedYaraMatches = analysis.yaraMatches
    ? analysis.yaraMatches.map((match) => {
        // If it's already an object with rule properties, use it as is
        if (typeof match === "object" && match !== null) {
          return match;
        }
        
        // If it's a string, create a basic rule object
        return {
          rule: match,
          description: "YARA rule match detected",
          tags: [],
          strings: []
        };
      })
        ...
        <div className="flex items-center gap-2">
          ...
          {processedYaraMatches.length > 0 && (
            <span className="text-xs px-2 py-1 bg-kali-gray-800 text-kali-neonred rounded-full flex items-center gap-1 border border-kali-neonred pulse-alert">
              ...
              {processedYaraMatches.length} threat
              {processedYaraMatches.length !== 1 ? "s" : ""} detected
            </span>
          )}
        </div>
        {open ? (
          <ChevronUp className="w-5 h-5 text-kali-neonblue" />
        ) : (
          <ChevronDown className="w-5 h-5 text-kali-neonblue" />
        )}
      </button>
      ...
      {open && (
        <div className="mt-4">
          ...
          <div className="tab-navigation">
            ...
            >
              YARA Rules ({processedYaraMatches.length || 0})
            </button>
          </div>
          ...
          <div className="bg-kali-gray-800 rounded-lg p-4 border border-kali-purple">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      <span className="text-kali-gray-400 font-mono">
                        File Size:
                      </span>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      ...
                      <span className="text-kali-neongreen font-mono text-xs">
                        {analysis.md5}
                      </span>
                    </div>
                    <div className="flex justify-between">
            ...
            {activeTab === "strings" && (
              <div>
                <h3 className="text-md font-semibold text-kali-neonblue mb-3 font-mono">
                  Extracted Strings
                </h3>
                ...
                {processedIocs.length > 0 ? (
                  <div className="space-y-3">
                    {processedIocs.map((ioc, index) => (
                      <div
                        key={index}
                        className="p-3 bg-kali-gray-700 rounded-lg border border-kali-purple"
                      >
                        <div className="font-mono text-sm text-kali-neonblue">
                          {ioc.type}
                        </div>
                        <div className="font-mono text-kali-neongreen text-xs break-all">
                          {ioc.value}
                        </div>
                        {ioc.context && (
                          <div className="font-mono text-kali-gray-400 text-xs mt-1">
                            {ioc.context}
                          </div>
                        )}
                      </div>
                  ...
                  <div className="text-center py-6">
                    <Terminal className="w-12 h-12 mx-auto mb-2 opacity-50 text-kali-neonblue" />
                    <p className="text-kali-gray-400 font-mono">
                      No IOCs detected
                    </p>
                  </div>
                )}
            ...
            {activeTab === "yara" && (
              <div>
                ...
                {processedYaraMatches.length > 0 ? (
                  <div className="space-y-3">
                    {processedYaraMatches.map((match, index) => (
                      <div key={index} className="yara-match p-4 rounded-lg">
                        <div className="flex items-center gap-2 mb-2 yara-match-header">
                        ...
                        {match.tags && match.tags.length > 0 && (
                          <div className="yara-tags">
                            {match.tags.map((tag, i) => (
                              <span key={i} className="yara-tag font-mono">
                                {tag}
                              </span>
                            ))}
                          </div>
                        ...
                        {match.strings && match.strings.length > 0 && (
                          <div>
                            ...
                            <div className="yara-strings">
                              {match.strings.slice(0, 5).map((str, i) => (
                                <div key={i} className="font-mono text-sm">
                                  {str}
                                </div>
                              ))}
                              {match.strings.length > 5 && (
                                <div className="text-xs text-gray-500 mt-1">
                                  + {match.strings.length - 5} more strings
                                </div>
                              )}
                            </div>
                          </div>
